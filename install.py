...
def make_url():
    return f"vless://{state['uid']}@{DOMAIN}:443?security=tls&encryption=none&type=ws&path={state['path']}&host={DOMAIN}&sni={DOMAIN}&fp=chrome#STARGATE"
...
<style>body{{font-family:system-ui;background:#0a0a0a;color:#c9d1d9;padding:20px;text-align:center}}
code{{background:rgba(0,0,0,.5);padding:10px;display:block;border-radius:8px;word-break:break-all;color:#3fb950;font-size:.8em;margin:10px 0}}
...
<h1>🌀 Stargate VLESS</h1><p>{DOMAIN}</p><code>{state['url']}</code>
<button class="btn" onclick="navigator.clipboard.writeText('{state['url']}');alert('Copied!')">📋 Copy</button>
<button class="btn" onclick="fetch('/api/new').then(r=>r.json()).then(d=>{{document.querySelector('code').textContent=d.url}})">🔄 New</button>
</body></html>"""
            ...
            self.wfile.write(json.dumps({'url':state['url'],'domain':DOMAIN,'uuid':state['uid'],'path':state['path']}).encode())
            ...
            self.send_response(200); self.send_header('Content-Type','application/json'); self.end_headers()
            self.wfile.write(b'{"name":"Stargate VLESS","short_name":"Stargate","start_url":"/","display":"standalone","background_color":"#0a0a0a","theme_color":"#0a0a0a"}')
